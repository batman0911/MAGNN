...
def run_model_LastFM(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    for _ in range(repeat):
        ...
        # send data to GPU
        # train_pos_idx_batch_mapped_list = [torch.LongTensor(indices).to(device) for indices in train_pos_idx_batch_mapped_list]
        
        # train_pos_g_list = [g.to(device) for g in train_pos_g_list]
        
        # train_neg_idx_batch_mapped_list = [torch.LongTensor(indices).to(device) for indices in train_neg_idx_batch_mapped_list]
        
        # train_neg_g_list = [g.to(device) for g in train_neg_g_list]
        ...
        #  send data to GPU
        # val_pos_idx_batch_mapped_list = [torch.LongTensor(indices).to(device) for indices in val_pos_idx_batch_mapped_list]
        
        # val_pos_g_list = [g.to(device) for g in val_pos_g_list]
        
        # val_neg_idx_batch_mapped_list = [torch.LongTensor(indices).to(device) for indices in val_neg_idx_batch_mapped_list]
        
        # val_neg_g_list = [g.to(device) for g in val_neg_g_list]
        
        print(f'val_pos_g_lists: {val_pos_g_lists[0][0].device}, val_pos_indices_lists: {val_pos_indices_lists[0][0].is_cuda}, val_pos_idx_batch_mapped_lists: {val_pos_idx_batch_mapped_lists[0][0].is_cuda}')
